...
def _determine_slashing_penalty(
    total_penalties: Gwei, total_balance: Gwei, balance: Gwei
) -> Gwei:
    return Gwei(balance * min(total_penalties * 3, total_balance) // total_balance)
    ...
    for index, validator in enumerate(state.validators):
        ...
        if (
            ...
            penalty = _determine_slashing_penalty(
                Gwei(sum(state.slashings)), total_balance, validator.effective_balance
            )
